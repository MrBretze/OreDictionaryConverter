...
public class TileEntityAutomaticOreConverter extends TileEntity implements ITickable, IInventory
{
    ...
    private void writeItemToNBT(NBTTagCompound compound, int slot)
    {
        ...
        is.writeToNBT(tag);
        compound.setTag("Item_" + slot, tag);
    }
    ...
    private void writeTargetToNBT(NBTTagCompound compound, int slot)
    {
        ...
        NBTTagCompound tag = new NBTTagCompound();
        is.writeToNBT(tag);
        compound.setTag("Target_" + slot, tag);
    }
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        ...
        {
            NBTTagCompound tag = (NBTTagCompound) compound.getTag("Item_" + i);
            if (tag != null)
            {
        ...
        {
            NBTTagCompound tag = (NBTTagCompound) compound.getTag("Target_" + i);
            if (tag != null)
            {
                ItemStack stack;
    ...
    {
        return world.getTileEntity(pos) == this && getDistanceSq(player.posX, player.posY, player.posZ) <= 64.0D;
    }
